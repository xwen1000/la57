...
class CreateOrdersTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('orders', function (Blueprint $table) {
            ...
            $table->string('order_sn', 50)->default('');
            ...
            $table->string('shipping_name', 20)->default('');
            $table->string('shipping_code', 20)->default('');
            $table->string('tracking_number', 80)->default('');
            $table->tinyInteger('order_status')->default(0);
            $table->integer('order_time')->default(0);
            $table->integer('pay_time')->default(0);
            $table->integer('received_time')->default(0);
            $table->date('start_date')->nullable()->comment('预定房间的起始时间');
            $table->date('end_date')->nullable()->comment('预定房间的结束时间');
            $table->string('book_name')->nullable()->comment('预订房间人的姓名');
            $table->string('book_phone', 20)->nullable()->comment('预订房间预留电话号码');
